...
export default async function SettingsPage() {
  ...
  const { data: capitalProfile } = await supabase
    .from("profiles")
    .select(
      "starting_capital, current_capital, risk_per_trade_percent, daily_profit_target_percent, weekly_profit_target_amount, daily_loss_limit_percent, max_consecutive_losses",
    )
    .eq("id", auth.user.id)
    .maybeSingle();
