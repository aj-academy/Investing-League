...
export async function GET(request: Request) {
  ...
  const { data: profiles } = await admin
    .from("profiles")
    .select(
      "id, full_name, email, plan, starting_capital, current_capital, risk_per_trade_percent, daily_profit_target_percent, daily_profit_target_amount, daily_loss_limit_percent, max_consecutive_losses",
    )
