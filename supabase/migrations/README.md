...
Recommended order: `fix_missing_profiles` → `profiles_rls_insert` → `subscription_plans` → `admin_core_phase1` → `phase2_terms_assets` → `pricing_plans` (or run only what you need).
